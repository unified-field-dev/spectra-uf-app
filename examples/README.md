...
Runnable teaching hosts for this UF app. Each card: when to use · command · success · look next.

## Canonical path

### `protected-spectra-host` — auth + `/spectra` schema index

**Teaches:** session auth gate on `/spectra` and the schema catalog `spectra-backend` exposes for the UI index.
...
**Next step:** Mount `<SpectraRoutes />` in a product host with Spectra query backends.
